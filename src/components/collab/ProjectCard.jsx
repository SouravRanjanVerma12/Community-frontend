...
const DOMAIN_COLORS = {
  webdev:  { bg: 'var(--accent-bg, rgba(30,157,241,0.12))',  border: 'var(--accent-border, rgba(30,157,241,0.28))',  text: 'var(--accent, #1e9df1)' },
  ...
  default: { bg: 'var(--accent-bg, rgba(30,157,241,0.12))',  border: 'var(--accent-border, rgba(30,157,241,0.28))',  text: 'var(--accent, #1e9df1)' },
};
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-3 p-5 flex-1">
          ...
          {post.projectName && (
            <div className="flex flex-col gap-[2px]">
              ...
              <p className="text-[13px] font-bold m-0 text-accent">{post.projectName}</p>
            </div>
          )}

          {/* Title */}
          <h3 className="text-[15px] font-bold text-text-primary leading-snug tracking-tight m-0 group-hover:text-accent transition-colors duration-150">
            {post.title}
          </h3>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-2 mt-auto px-5 py-3 border-t border-border/40 flex-wrap">
          {/* Author */}
          <Link
            to={`/profile/${post.author?._id}`}
            className="flex items-center gap-2 no-underline shrink-0 group/avatar z-10 min-w-0 max-w-[45%]"
            onClick={e => e.stopPropagation()}
          >
            <Avatar name={post.author?.name} src={post.author?.avatarUrl} size={24} />
            <span className="text-[12px] font-medium text-text-secondary group-hover/avatar:text-text-primary transition-colors truncate">
              {post.author?.name}
            </span>
          </Link>

          {/* Right Action & Member Count */}
          <div className="flex items-center gap-2 shrink-0 ml-auto z-10 flex-wrap" onClick={e => e.stopPropagation()}>
            {/* Member count */}
            <span
            ...
            {isOwn ? (
              showReviewLink && (post.requestCount ?? 0) > 0 ? (
                <Link to={`/collab/${post._id}/requests`} className="no-underline block">
                  <motion.span
                    ...
                    whileTap={{ scale: 0.96 }}
                    className="flex items-center gap-1.5 text-[11px] font-semibold px-3 py-1.5 rounded-lg text-white cursor-pointer bg-(image:--btn-grad) shadow-btn"
                  >
                ...
                <span
                  className="text-[11px] font-semibold px-2.5 py-1.5 rounded-lg text-accent bg-accent-bg border border-accent-border"
                >
                  Your collab
                </span>
              )
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.04 }}
                whileTap={{ scale: 0.96 }}
                className="flex items-center gap-1.5 text-[12px] font-bold px-3.5 py-1.5 rounded-lg text-white border-none cursor-pointer bg-(image:--btn-grad) shadow-btn"
              >
                Apply
              </motion.button>
            ) : (
